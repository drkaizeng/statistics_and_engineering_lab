...
- For each of the $L_{\text{IV}}$ loci, 
    ...
    - Compute the $p$-value for testing the null hypothesis of no association (i.e., $\beta_{GX}^{(i)} = 0$ and $\beta_{GY}^{(i)} = 0$) by using the fact that $z_{GX}^{(i)2}$ and $z_{GY}^{(i)2}$ follow a chi-squared distribution with 1 degree of freedom under the null hypothesis.
